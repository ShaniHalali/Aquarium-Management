...
void writeRandomFishToBinaryFile(RandomFish* random, FILE* file) {
    ...
    FishTimestamp* timestamp = (FishTimestamp*)random->extraInfo;
    if (timestamp != NULL) {
        fwrite(&timestamp->timestamp, sizeof(time_t), 1, file);
    }
    else {
        time_t defaultTime = 0;
        fwrite(&defaultTime, sizeof(time_t), 1, file);
    }
}
...
RandomFish* readRandomFishFromBinaryFile(FILE* file) {
    ...
    if (fread(&timestamp->timestamp, sizeof(time_t), 1, file) != 1) {
        free(timestamp);
        free(random);
        return NULL;
    }
...
void printRandomFishesLinkedList(RandomFish* head) {
