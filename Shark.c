...
void getSharkFromUser(Shark* shark) {
    do {
        printf("Enter shark name (only letters and numbers are allowed): ");
        scanf("%s", shark->name);
        if (isValidName(shark->name)) {
            break;
        }
        printf("Invalid name. Name must contain only English letters and numbers.\n");
    } while (1);
    ...
    getValidColors(&shark->seaCreature);
}
